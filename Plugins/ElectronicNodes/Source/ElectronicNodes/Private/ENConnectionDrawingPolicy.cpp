...
FConnectionDrawingPolicy* FENConnectionDrawingPolicyFactory::CreateConnectionPolicy(const class UEdGraphSchema* Schema, int32 InBackLayerID,
	int32 InFrontLayerID, float ZoomFactor, const class FSlateRect& InClippingRect, class FSlateWindowElementList& InDrawElements,
	class UEdGraph* InGraphObj) const
{
	...
	if (ElectronicNodesSettings.ActivateOnAnimation)
	{
		...
		{
			return new FENAnimGraphConnectionDrawingPolicy(
	...
	{
		return new FENBehaviorTreeConnectionDrawingPolicy(
			InBackLayerID, InFrontLayerID, ZoomFactor, InClippingRect, InDrawElements, InGraphObj);
	}

	if (ElectronicNodesSettings.ActivateOnControlRig && ClassName == "ControlRigGraphSchema")
	{
		return new FENControlRigConnectionDrawingPolicy(
			InBackLayerID, InFrontLayerID, ZoomFactor, InClippingRect, InDrawElements, InGraphObj);
	}
...
void FENConnectionDrawingPolicy::DrawConnection(
	int32 LayerId, const FVector2D& Start, const FVector2D& End, const FConnectionParams& Params)
{
	...
	switch (WireStyle)
	{
		case EWireStyle::Manhattan: PathDrawer.DrawManhattanWire(NewStart, StartDirection, NewEnd, EndDirection); break;
		case EWireStyle::Subway: PathDrawer.DrawSubwayWire(NewStart, StartDirection, NewEnd, EndDirection); break;
		default: PathDrawer.DrawDefaultWire(NewStart, StartDirection, NewEnd, EndDirection);
	}
		...
		if (bCloseToSpline)
		{
			if (ClosestDistanceSquared < SplineOverlapResult.GetDistanceSquared())
			{
				...
				SplineOverlapResult = FGraphSplineOverlapResult(
					Params.AssociatedPin1, Params.AssociatedPin2, ClosestDistanceSquared, SquaredDistToPin1, SquaredDistToPin2);
			}
		}
	}
}
...
void FENConnectionDrawingPolicy::ENProcessRibbon(
	int32 LayerId, FVector2D& Start, FVector2D& StartDirection, FVector2D& End, FVector2D& EndDirection, const FConnectionParams& Params)
{
	...
	if (WirePriority != EWirePriority::None)
	{
		if ((Params.AssociatedPin1 != nullptr) && (Params.AssociatedPin2 != nullptr))
		{
			...
			if (IsOutputPin ^ IsInputPin)
			{
				switch (WirePriority)
				{
					case EWirePriority::Node: RightPriority = IsOutputPin; break;
					case EWirePriority::Pin: RightPriority = IsInputPin; break;
					default: break;
				}
			}
		}
...
{
	const float ZoomLevels[] = {2.0f, 1.875f, 1.75f, 1.675f, 1.5f, 1.375f, 1.25f, 1.0f, 0.875f, 0.75f, 0.675f, 0.5f, 0.375f, 0.25f, 0.225f,
...
void FENConnectionDrawingPolicy::BuildRelatedNodes(
	UEdGraphNode* Node, TArray<UEdGraphNode*>& RelatedNodes, bool InputCheck = true, bool OutputCheck = true)
{
...
void FENConnectionDrawingPolicy::ENDrawBubbles(
	const FVector2D& Start, const FVector2D& StartTangent, const FVector2D& End, const FVector2D& EndTangent)
{
	const bool ENDrawBubbles =
		ElectronicNodesSettings.ForceDrawBubbles && (ElectronicNodesSettings.BubbleZoomThreshold <= ENGetZoomLevel());
	if (_Params->bDrawBubbles || ENDrawBubbles)
	{
		...
		FVector2D BubbleSize =
			BubbleImage->ImageSize * ZoomFactor * 0.1f * ElectronicNodesSettings.BubbleSize * FMath::Sqrt(_Params->WireThickness);
		if (_Params->bDrawBubbles)
		{
			BubbleSize *= 1.25f;
		}
		...
		for (int32 i = 0; i < NumBubbles; ++i)
		{
			...
			FSlateDrawElement::MakeBox(DrawElementsList, _LayerId, FPaintGeometry(BubblePos, BubbleSize, ZoomFactor), BubbleImage,
				ESlateDrawEffect::None, _Params->WireColor);
		}
	}
}
...
void FENConnectionDrawingPolicy::ENDrawArrow(const FVector2D& Start, const FVector2D& End)
{
	if (MidpointImage != nullptr && (Start - End).Size() > 4 * MinXOffset)
	{
		...
		FSlateDrawElement::MakeRotatedBox(DrawElementsList, _LayerId,
			FPaintGeometry(MidpointDrawPos, MidpointImage->ImageSize * ZoomFactor * 0.75f, ZoomFactor * 0.75f), MidpointImage,
			ESlateDrawEffect::None, AngleInRadians, TOptional<FVector2D>(), FSlateDrawElement::RelativeToElement, _Params->WireColor);
	}
}

void FENConnectionDrawingPolicy::DrawDebugPoint(const FVector2D& Position, FLinearColor Color)
{
	const FVector2D BubbleSize =
		BubbleImage->ImageSize * ZoomFactor * 0.1f * ElectronicNodesSettings.BubbleSize * FMath::Sqrt(_Params->WireThickness);
	const FVector2D BubblePos = Position - (BubbleSize * 0.5f);

	FSlateDrawElement::MakeBox(
		DrawElementsList, _LayerId, FPaintGeometry(BubblePos, BubbleSize, ZoomFactor), BubbleImage, ESlateDrawEffect::None, Color);
}
...
void FENConnectionDrawingPolicy::ENComputeClosestPointDefault(
	const FVector2D& Start, const FVector2D& StartTangent, const FVector2D& End, const FVector2D& EndTangent)
{
	...
	if (bCloseToSpline)
	{
		...
		{
			const FVector2D PointOnSpline2 =
				FMath::CubicInterp(Start, StartTangent * Tangent, End, EndTangent * Tangent, TestAlpha + StepInterval);
