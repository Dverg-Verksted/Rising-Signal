...
{
public:
	FENBehaviorTreeConnectionDrawingPolicy(int32 InBackLayerID, int32 InFrontLayerID, float ZoomFactor, const FSlateRect& InClippingRect,
		...
		: FBehaviorTreeConnectionDrawingPolicy(InBackLayerID, InFrontLayerID, ZoomFactor, InClippingRect, InDrawElements, InGraphObj)
	{
		this->ConnectionDrawingPolicy =
			new FENConnectionDrawingPolicy(InBackLayerID, InFrontLayerID, ZoomFactor, InClippingRect, InDrawElements, InGraphObj, true);
	}
	...
	virtual void DrawSplineWithArrow(const FGeometry& StartGeom, const FGeometry& EndGeom, const FConnectionParams& Params) override
	{
		...
		const FVector2D StartCenter =
			FVector2D(StartGeom.AbsolutePosition.X + StartGeomDrawSize.X / 2, StartGeom.AbsolutePosition.Y + StartGeomDrawSize.Y);

		const FVector2D EndGeomDrawSize = EndGeom.GetDrawSize();
		const FVector2D EndCenter = FVector2D(EndGeom.AbsolutePosition.X + EndGeomDrawSize.X / 2, EndGeom.AbsolutePosition.Y);

		DrawSplineWithArrow(StartCenter, EndCenter, Params);
	}

	virtual void DrawPreviewConnector(
		const FGeometry& PinGeometry, const FVector2D& StartPoint, const FVector2D& EndPoint, UEdGraphPin* Pin) override
	{
		...
		if (Pin->Direction == EEdGraphPinDirection::EGPD_Output)
		{
			...
			const FVector2D Center =
				FVector2D(PinGeometry.AbsolutePosition.X + GeomDrawSize.X / 2, PinGeometry.AbsolutePosition.Y + GeomDrawSize.Y);
		...
		{
			const FVector2D GeomDrawSize = PinGeometry.GetDrawSize();
			const FVector2D Center = FVector2D(PinGeometry.AbsolutePosition.X + GeomDrawSize.X / 2, PinGeometry.AbsolutePosition.Y);
	...
	virtual void DrawSplineWithArrow(
		const FVector2D& StartAnchorPoint, const FVector2D& EndAnchorPoint, const FConnectionParams& Params) override
	{
	...
	void Internal_DrawLineWithStraightArrow(
		const FVector2D& StartAnchorPoint, const FVector2D& EndAnchorPoint, const FConnectionParams& Params)
	{
		...
		FSlateDrawElement::MakeRotatedBox(DrawElementsList, ArrowLayerID,
			FPaintGeometry(ArrowDrawPos, ArrowImage->ImageSize * ZoomFactor, ZoomFactor), ArrowImage, ESlateDrawEffect::None, HALF_PI,
			TOptional<FVector2D>(), FSlateDrawElement::RelativeToElement, Params.WireColor);
	}

	~FENBehaviorTreeConnectionDrawingPolicy() { delete ConnectionDrawingPolicy; }
