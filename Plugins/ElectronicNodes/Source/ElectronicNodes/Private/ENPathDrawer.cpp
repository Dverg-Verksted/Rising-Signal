...
FENPathDrawer::FENPathDrawer(int32& LayerId, float& ZoomFactor, bool RightPriority, const FConnectionParams* Params,
	FSlateWindowElementList* DrawElementsList, FENConnectionDrawingPolicy* ConnectionDrawingPolicy)
{
...
void FENPathDrawer::DrawManhattanWire(
...
void FENPathDrawer::DrawSubwayWire(
	const FVector2D& Start, const FVector2D& StartDirection, const FVector2D& End, const FVector2D& EndDirection)
{
	if (!MaxDepthWire--)
	{
	...
	const bool StartDirection90 = FMath::IsNearlyEqual(FMath::Abs(StartDirection.X), 1.0f, KINDA_SMALL_NUMBER) ||
								  FMath::IsNearlyEqual(FMath::Abs(StartDirection.Y), 1.0f, KINDA_SMALL_NUMBER);
	const bool EndDirection90 = FMath::IsNearlyEqual(FMath::Abs(EndDirection.X), 1.0f, KINDA_SMALL_NUMBER) ||
								FMath::IsNearlyEqual(FMath::Abs(EndDirection.Y), 1.0f, KINDA_SMALL_NUMBER);
	...
	const float DirectionOffset = (FMath::Abs(End.X - Start.X) - FMath::Abs(End.Y - Start.Y)) *
								  (FMath::IsNearlyEqual(FMath::Abs(StartDirection.X), 1.0f, KINDA_SMALL_NUMBER) ? 1 : -1);
	...
	else if (SameDirection)
	{
		...
		else if (ForwardDirection && DirectionOffset > 2 * GetIntersectionOffset(45, false))
		{
			...
			{
				if ((FMath::Abs(DistanceOrtho) < 2 * GetIntersectionOffset(45, false)) &&
					(FMath::Abs(DistanceStraight) > 2 * GetRadiusOffset()))
				{
...
void FENPathDrawer::DrawDefaultWire(
	const FVector2D& Start, const FVector2D& StartDirection, const FVector2D& End, const FVector2D& EndDirection)
{
	const float Tangent = (End - Start).Size();

	FSlateDrawElement::MakeDrawSpaceSpline(*DrawElementsList, LayerId, Start, StartDirection * Tangent, End, EndDirection * Tangent,
...
void FENPathDrawer::DrawIntersectionRadius(
	const FVector2D& Start, const FVector2D& StartDirection, const FVector2D& End, const FVector2D& EndDirection)
{
	const int32 AngleDeg = FMath::RoundToInt(FMath::UnwindDegrees(
		FMath::RadiansToDegrees(FMath::Atan2(StartDirection.Y, StartDirection.X) - FMath::Atan2(EndDirection.Y, EndDirection.X))));

	const float StartOffset = GetIntersectionOffset(AngleDeg, false);
	const float EndOffset = GetIntersectionOffset(AngleDeg, true);

	const float T = (EndDirection.X * (Start.Y - End.Y) - EndDirection.Y * (Start.X - End.X)) /
					(-EndDirection.X * StartDirection.Y + StartDirection.X * EndDirection.Y);
...
void FENPathDrawer::DrawIntersectionDiagRadius(
	const FVector2D& Start, const FVector2D& StartDirection, const FVector2D& End, const FVector2D& EndDirection)
{
	const float DirectionOffset = FMath::Abs(End.X - Start.X) - FMath::Abs(End.Y - Start.Y);
...
void FENPathDrawer::DrawSimpleRadius(const FVector2D& Start, const FVector2D& StartDirection, const int32& AngleDeg, FVector2D& out_End,
	FVector2D& out_EndDirection, bool Backward)
{
...
void FENPathDrawer::DrawUTurn(const FVector2D& Start, const FVector2D& StartDirection, float Direction, FVector2D& out_End,
	FVector2D& out_EndDirection, bool Backward)
{
...
void FENPathDrawer::DrawCorrectionOrtho(const FVector2D& Start, const FVector2D& StartDirection, const float& Displacement,
	FVector2D& out_End, FVector2D& out_EndDirection, bool Backward)
{
	out_EndDirection = StartDirection;
...
float FENPathDrawer::GetRadiusOffset(const int32& AngleDeg, bool Perpendicular)
{
	...
	switch (AbsAngle)
	{
		case 45: RadiusOffset *= FMath::Sqrt(2.0f) / 2.0f; break;
		case 90: RadiusOffset *= 1.0f; break;
		case 135: RadiusOffset *= (1.0f - (FMath::Sqrt(2.0f) / 2.0f)); break;
		case 180: RadiusOffset *= 2.0f; break;
		default: break;
	}
...
float FENPathDrawer::GetRadiusTangent(const int32& AngleDeg)
{
	...
	switch (FMath::Abs(AngleDeg))
	{
		...
		case 90: Tangent = 4 * (FMath::Sqrt(2.0f) - 1); break;
...
float FENPathDrawer::GetIntersectionOffset(const int32& AngleDeg, bool Diagonal)
{
	...
	switch (FMath::Abs(AngleDeg))
	{
		case 45:
			...
			break;
		case 90: IntersectionOffset *= 1.0f; break;
		case 135:
...
void FENPathDrawer::DrawLine(const FVector2D& Start, const FVector2D& End)
{
	...
	FSlateDrawElement::MakeDrawSpaceSpline(*DrawElementsList, LayerId, Start, FVector2D::ZeroVector, End, FVector2D::ZeroVector,
		Params->WireThickness * ElectronicNodesSettings.WireThickness, ESlateDrawEffect::None, WireColor);
...
void FENPathDrawer::DrawRadius(
	const FVector2D& Start, const FVector2D& StartDirection, const FVector2D& End, const FVector2D& EndDirection, const int32& AngleDeg)
{
